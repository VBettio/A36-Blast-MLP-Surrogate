...
# ==========================================
# 1. ALGORITMO LATIN HYPERCUBE (DoE)
# ==========================================
def gerar_lhs(n, min_val, max_val):
    step = (max_val - min_val) / float(n)
    pontos = [min_val + i * step + random.uniform(0, step) for i in range(n)]
    random.shuffle(pontos)
    return pontos

# ==========================================
# 2. PARÂMETROS DO ESPAÇO DE PROJETO (1000 CASOS)
# ==========================================
N_SIMULACOES = 1000

massas_tnt = gerar_lhs(N_SIMULACOES, 0.1, 5)         # 0.5 kg a 3.0 kg
alturas_z = gerar_lhs(N_SIMULACOES, 0.1, 3.0)          # 0.1 m a 1.0 m
espessuras = gerar_lhs(N_SIMULACOES, 0.001, 0.008)     # 0.002 m a 0.008 m (1mm a 8mm)
...
# 3. EXECUÇÃO EM LOTE 
# ==========================================
with open('DoE_A36_TNT.csv', mode='w') as f:
    ...
    for i in range(N_SIMULACOES):
        ...
        nome_job = 'DoE_A36_{:03d}_M{}g_Z{}mm_e{}mm'.format(i+1, massa_g, dist_mm, esp_str)
